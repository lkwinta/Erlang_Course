require Pollutiondbweb.Reading
require Pollutiondbweb.Station
...
defmodule PollutionDataLoader do
  ...
  def upload_stations(readings) do
    readings
      |> identify_stations()
      |> Enum.map(&Pollutiondbweb.Station.add("#{&1.id} #{&1.name}", &1.location.lon, &1.location.lat))
  end
  ...
  defp add_reading(reading) do
    Pollutiondbweb.Station.find_by_name("#{reading.id} #{reading.name}")
      |> Enum.at(0)
      |> Pollutiondbweb.Reading.add(reading.date_time.date, reading.date_time.time, reading.type, reading.value)
  end
